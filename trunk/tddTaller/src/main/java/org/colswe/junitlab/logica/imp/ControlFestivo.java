...
public class ControlFestivo implements IControlFestivo {
	...
	/**
	 * Número de horas de trabajo de un viernes de quincena.
	 */
	private static final int NUM_HORAS_VIERNES_QUINCENA = 2;
	...
	public Integer contarHoras(final Date desde, final Date hasta) {
		...
		if (fechasValidas(desde, hasta)) {
			...
			horas += ret.get(TipoDia.VIERNES_QUINCENA)
					* NUM_HORAS_VIERNES_QUINCENA;
		}
		return horas;
	}
	...
	public Map<TipoDia, Integer> obtenerDias(final Date desde, final Date hasta) {
		...
		ret.put(TipoDia.VIERNES_QUINCENA, 0);
		...
		while (cH.after(c)) {
			...
			if (!festivo && c.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY) {
				sabado++;
					...
					&& (c.get(Calendar.DAY_OF_WEEK) != Calendar.SUNDAY || c
				...
				if (c.get(Calendar.DAY_OF_MONTH) >= 12
					...
					if (c.get(Calendar.DAY_OF_MONTH) >= 25) {
						numDiasMes = c.getActualMaximum(Calendar.DAY_OF_MONTH);
					}

					int diaMenor = 0;
					final int diaMayor = 3;
					...
					if (c.get(Calendar.DAY_OF_MONTH) >= diaMenor) {
						viernesQuincena++;
					} else if (c.get(Calendar.DAY_OF_MONTH) <= diaMayor) {
						viernesQuincena++;
					} else {
						normal++;
					}
		...
		ret.put(TipoDia.VIERNES_QUINCENA, viernesQuincena);
		return ret;
	}
	 ...
	 * @return 1 si la fecha es domingo no laborable, 0 en otro caso.
	 */
	private int domingoNoLaborable(final Calendar ct) {
		int ret;
		if (ct.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY) {
			ret = 0;
		} else {
			ret = 1;
		}
